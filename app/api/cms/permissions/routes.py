...
from app.models.cms.models import CMSSystemModule, CMSUser
...
async def get_accessible_departments(
    ...
    return {
        ...
        "user_role": current_user.role,
        "user_department_id": str(current_user.department_id)
...
async def initialize_default_permissions(
    current_user: CMSUser = Depends(get_principal_user), db: Session = Depends(get_db)
):
