...
async def generate_presigned_url_endpoint(request: PresignedUrlRequest):
    ...
    try:
        ...
        return PresignedUrlResponse(
            status_code=500, message="Error generating presigned URL", body=None
        )

    except KeyError as e:
        logger.error("KeyError: %s", e)
        return PresignedUrlResponse(
            status_code=400, message="Missing required key", body=None
        )

    except Exception as e:
        logger.error("Error: %s", e)
        return PresignedUrlResponse(
            status_code=500, message="Internal server error", body=None
        )
