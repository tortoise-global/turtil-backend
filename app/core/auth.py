...
def require_module_access(module_name: str, action: str = 'read'):
    ...
    async def module_checker(
        ...
        if not permission_service.has_module_access(current_user, module_name, action):
            logger.warning(f"Module access denied for user {current_user.id} to {module_name}:{action}")
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    async def department_checker(
        ...
        if target_department_id is None:
            ...
            if not accessible_departments:
                ...
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="No department access"
                )
        else:
            if not permission_service.can_access_department_data(current_user, target_department_id):
                logger.warning(f"Department access denied for user {current_user.id} to dept {target_department_id}")
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
...
def require_branch_access(target_branch_id: Optional[UUID] = None):
    ...
    async def branch_checker(
        ...
        if target_branch_id is None:
            accessible_branches = permission_service.get_accessible_branches(current_user)
            ...
            if not permission_service.can_access_branch_data(current_user, target_branch_id):
                logger.warning(f"Branch access denied for user {current_user.id} to branch {target_branch_id}")
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
...
def require_module_and_scope_access(module_name: str, action: str = 'read', scope_check: str = None):
