...
from app.models.cms.models import (
    CMSUser, CMSSystemModule, CMSUserModulePermission, 
    CMSRoleModuleAccess, CMSCollegeModule, Department, Branch
)
...
class PermissionService:
    ...
    DEFAULT_PERMISSIONS = {
        'principal': {
            'programs_structure': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'all'},
            'students': {'actions': ['read', 'write', 'delete', 'manage', 'export', 'import'], 'scope': 'all'},
            'lists': {'actions': ['read', 'write', 'delete'], 'scope': 'all'},
            'alerts': {'actions': ['read', 'write', 'delete'], 'scope': 'all'},
            'attendance': {'actions': ['read', 'write', 'delete', 'manage', 'export', 'import'], 'scope': 'all'},
            'timetable': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'all'},
            'results': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'all'},
            'assignments': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'all'},
            'academic_calendar': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'all'},
            'document_request': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'all'},
            'events': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'all'},
            'placements': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'all'},
            'account': {'actions': ['read', 'write', 'manage'], 'scope': 'all'},
        },
        'admin': {
            'programs_structure': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'college'},
            'students': {'actions': ['read', 'write', 'delete', 'manage', 'export', 'import'], 'scope': 'college'},
            'lists': {'actions': ['read', 'write', 'delete'], 'scope': 'college'},
            'alerts': {'actions': ['read', 'write', 'delete'], 'scope': 'college'},
            'attendance': {'actions': ['read', 'write', 'delete', 'manage', 'export', 'import'], 'scope': 'college'},
            'timetable': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'college'},
            'results': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'college'},
            'assignments': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'college'},
            'academic_calendar': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'college'},
            'document_request': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'college'},
            'events': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'college'},
            'placements': {'actions': ['read', 'write', 'delete', 'manage'], 'scope': 'college'},
        ...
        'head': {
            ...
            'attendance': {'actions': ['read', 'write', 'export', 'import'], 'scope': 'department'},
        ...
        'staff': {
            ...
            'attendance': {'actions': ['read', 'write', 'export', 'import'], 'scope': 'branch'},
    ...
    ROLE_MODULE_VISIBILITY = {
        ...
        'staff': ['programs_structure', 'timetable', 'document_request', 'events', 'placements'],
    ...
    def has_module_access(self, user: CMSUser, module_name: str, action: str = 'read') -> bool:
        ...
        try:
            ...
            if user_permission:
                return (user_permission.has_access and 
                       action in (user_permission.allowed_actions or []))
        ...
        try:
            ...
            for module in modules:
                if self.has_module_access(user, module.name):
                    permission_details = self._get_module_permission_details(user, module.name)
    ...
    def can_access_department_data(self, user: CMSUser, target_department_id: UUID) -> bool:
        ...
        try:
            ...
            if user.role == 'admin':
                return user.college_id == self._get_department_college_id(target_department_id)
    ...
    def get_accessible_departments(self, user: CMSUser) -> List[UUID]:
        ...
        try:
            ...
            if user.role == 'admin':
                ...
                return [dept.id for dept in self.db.query(Department)
                       .filter(Department.college_id == user.college_id).all()]
            ...
            logger.error(f"Error getting accessible departments for user {user.id}: {e}")
            return []
    ...
    def get_accessible_branches(self, user: CMSUser) -> List[UUID]:
        ...
        try:
            if user.role in ['principal', 'admin']:
                ...
                return [branch.id for branch in self.db.query(Branch)
                       .filter(Branch.department_id.in_(accessible_dept_ids)).all()]
            
            if user.role == 'head':
                # All branches in their department
                return [branch.id for branch in self.db.query(Branch)
                       .filter(Branch.department_id == user.department_id).all()]
    ...
    def get_modules_for_role_creation(self, creating_user: CMSUser, target_role: str) -> List[Dict[str, Any]]:
        """Get modules visible when creating sub-accounts for a specific role"""
        try:
            # Get all enabled modules for the college
            college_modules = self.db.query(CMSSystemModule, CMSCollegeModule)\
                .join(CMSCollegeModule, CMSSystemModule.id == CMSCollegeModule.module_id)\
                .filter(CMSCollegeModule.college_id == creating_user.college_id)\
                .filter(CMSCollegeModule.is_enabled == True).all()
            ...
            for system_module, college_module in college_modules:
                if system_module.name not in hidden_modules:
                    ...
                    default_perms = self.DEFAULT_PERMISSIONS.get(target_role, {}).get(system_module.name, {})
                    ...
                    visible_modules.append({
                        ...
                        'can_modify': creating_user.role in ['principal', 'admin']  # Principal and admin can modify permissions
                    })
    ...
    def create_default_permissions_for_user(self, user: CMSUser) -> bool:
        ...
        try:
            ...
            for module_name, permissions in role_permissions.items():
                module = self.db.query(CMSSystemModule).filter(CMSSystemModule.name == module_name).first()
                if module:
                    # Check if permission already exists
                    existing = self.db.query(CMSUserModulePermission)\
                        .filter(CMSUserModulePermission.user_id == user.id)\
                        .filter(CMSUserModulePermission.module_id == module.id).first()
    ...
    def _is_module_enabled_for_college(self, college_id: UUID, module_name: str) -> bool:
        """Check if module is enabled for the college"""
        return self.db.query(CMSCollegeModule)\
            ...
            .filter(CMSCollegeModule.is_enabled == True).first() is not None

    def _get_user_module_permission(self, user_id: UUID, module_name: str) -> Optional[CMSUserModulePermission]:
        """Get user's specific module permission"""
        return self.db.query(CMSUserModulePermission)\
            .join(CMSSystemModule, CMSUserModulePermission.module_id == CMSSystemModule.id)\
            .filter(CMSUserModulePermission.user_id == user_id)\
            .filter(CMSSystemModule.name == module_name).first()

    def _has_default_role_permission(self, role: str, module_name: str, action: str) -> bool:
    ...
    def _get_module_permission_details(self, user: CMSUser, module_name: str) -> Dict[str, Any]:
