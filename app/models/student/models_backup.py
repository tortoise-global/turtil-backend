...
class StudentUser(Base):
    ...
    assignment_id = Column(
        UUID(as_uuid=True),
        ForeignKey("cms_assignments.id", ondelete="CASCADE"),
        nullable=False,
    )
    student_id = Column(
        UUID(as_uuid=True),
...
class StudentResult(Base):
    ...
    examination_id = Column(
        UUID(as_uuid=True),
        ForeignKey("cms_examinations.id", ondelete="CASCADE"),
        nullable=False,
    )
    student_id = Column(
        UUID(as_uuid=True),
...
class StudentPlacement(Base):
    ...
    placement_drive_id = Column(
        UUID(as_uuid=True),
        ForeignKey("cms_placement_drives.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class StudentNotification(Base):
    ...
    notification_id = Column(
        UUID(as_uuid=True),
        ForeignKey("cms_notifications.id", ondelete="CASCADE"),
        nullable=False,
    )
    student_id = Column(
        UUID(as_uuid=True),
...
class StudentEventRegistration(Base):
    ...
    event_id = Column(
        UUID(as_uuid=True),
        ForeignKey("cms_events.id", ondelete="CASCADE"),
        nullable=False,
    )
    student_id = Column(
        UUID(as_uuid=True),
...
class StudentTimetablePreference(Base):
    ...
    timetable_slot_id = Column(
        UUID(as_uuid=True),
        ForeignKey("cms_timetable_slots.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class StudentDashboard(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    student_id = Column(
        UUID(as_uuid=True), ForeignKey("student_users.id"), nullable=False
    )
...
def get_student_attendance_summary(db, student_id: str, subject_id: str = None):
    ...
    query = (
        db.query(
            ...
            sql_func.sum(StudentAttendance.is_present.cast(Integer)).label(
                "present_sessions"
            ),
        )
        .join(StudentUser)
        .filter(StudentUser.id == student_id)
    )
...
def get_student_pending_assignments(db, student_id: str):
    ...
    submitted_assignment_ids = (
        db.query(StudentAssignmentSubmission.assignment_id)
        .filter(StudentAssignmentSubmission.student_id == student_id)
        .subquery()
    )

    return (
        db.query(CMSAssignment)
        .filter(
            and_(
