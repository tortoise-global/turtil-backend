...
class CMSUserPermission(BaseModel):
    """CMS User Permission model for module-based access control"""
    __tablename__ = "cms_user_permissions"
    
    # Foreign Keys
    cms_user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint('cms_user_id', 'module', name='unique_user_module_permission'),
    )
    
    def __repr__(self):
        return f"<CMSUserPermission(cms_user_id={self.cms_user_id}, module={self.module}, read={self.read_access}, write={self.write_access})>"
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "id": base_dict["id"],
            "cmsUserId": base_dict["cms_user_id"],
...
class CMSModules:
    ...
    ALL_MODULES = [
        PROGRAMS_STRUCTURE, STUDENTS, LISTS, ALERTS, TIMETABLE,
        ATTENDANCE, RESULTS, ASSIGNMENTS, ACADEMIC_CALENDAR,
        DOCUMENT_REQUEST, EVENTS, PLACEMENTS
    ]
...
class CMSRoles:
    ...
    USER_MANAGEMENT_ROLES = [PRINCIPAL, COLLEGE_ADMIN]
